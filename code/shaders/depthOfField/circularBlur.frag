...
void main()
{
	...
	{
		color += texture(tex, NDC + (vec2(weight1[i], weight1[i+1])*aspectcorrect));
	}
	
	for (int i=0; i < 16 / 2; i+=2) 
	{
		color += texture(tex, NDC + (vec2(weight2[i], weight2[i+1])*aspectcorrect) * 0.9);
	}
	
	for (int i=0; i < 16 / 2; i+=2) 
	{
		color += texture(tex, NDC + (vec2(weight3[i], weight3[i+1])*aspectcorrect) * 0.7);
		color += texture(tex, NDC + (vec2(weight3[i], weight3[i+1])*aspectcorrect) * 0.4);
	}  
